		...
		$.ajax({
			...
			, success : function(result){
				...
				$.each(result, function(index, place){
					...
					const bookmark = place.user_id && place.bookmarked
							? `<button class="bookmark" type="button">
...
//	    alert(dest_id);
	    ...
	    if ($(this).hasClass('ri-heart-line')) {
	        ...
	        $.ajax({
	        	...
	        	, success : function(){
	        		bookmark.removeClass('ri-heart-line').addClass('ri-heart-fill text-red-500');
	    	...
	    	$.ajax({
	    		...
	    		, data : {user_id : userId , dest_id : dest_id}
	    		, success  : function(){
	    			bookmark.removeClass('ri-heart-fill text-red-500').addClass('ri-heart-line');	    			
	    		}
	    		, error : function(err){
	    			console.log(err);
	    		}
